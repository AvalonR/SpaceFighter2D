...
void Bullet::SpawnSpecificBullet(float entityCenterX, float entityCenterY,
                                 double angleRad, double angleRadians,
                                 int delayMS, float damage) {
  float spawnDistance = 100.0f;
  Bullet bullet;
  bullet.dstR->x =
      entityCenterX + cos(angleRad) * spawnDistance - bullet.dstR->w / 2;
  bullet.dstR->y =
      entityCenterY + sin(angleRad) * spawnDistance - bullet.dstR->h / 2;
  ...
  if (bullet.dstR->x < -200 || bullet.dstR->y < -200 || bullet.dstR->x > 2020 ||
      bullet.dstR->y > 1920) {
...
void Bullet::spawnBulletPlayer() {
  ...
  if (!Player::PlayerUpgrades.firemodes[0] &&
      !Player::PlayerUpgrades.firemodes[1] &&
      !Player::PlayerUpgrades.firemodes[2]) {
...
void Bullet::spawnBulletEnemy(Vector Entity_destxy, Vector Entity_destwh,
                              double enemy_rotation, uint32_t EUID) {
  Bullet bullet;
  ...
  bullet.dstR->x =
      entityCenterX + cos(angleRadians) * spawnDistance -
      bullet.dstR->w / 2; // Subtract half the bullet's width to center it
  bullet.dstR->y =
      entityCenterY + sin(angleRadians) * spawnDistance -
      bullet.dstR->h / 2; // Subtract half the bullet's height to center it
  bullet.dstR->w = bullet.dstR->h = 64;
  ...
  if (bullet.dstR->x < 0 || bullet.dstR->y < 0 || bullet.dstR->x > 1920 ||
      bullet.dstR->y > 1920) {
    // SDL_Log("Error: Bullet spawned with invalid coordinates!");
  } else {
    bullet.owner = EUID;
    bullet.damage = (0.05f * GameManagerSingleton::instance().getDifficulty());
    BulletList.emplace_back(bullet);
    Sound::PlaySound(1);
  }
}
int SoundCooldownBattlecruiser = 0;
void Bullet::SpawnBattlecruiserBullet(float entityCenterX, float entityCenterY,
                                      double angleRad, double angleRadians,
                                      int delayMS, float damage,
                                      uint32_t EUID) {
  float spawnDistance = 100.0f;
  ...
  bullet.velocity.x = bulletSpeed * std::cos(angleRadians);
  bullet.velocity.y = bulletSpeed * std::sin(angleRadians);
  if (bullet.dstR->x < -100 || bullet.dstR->y < -100 || bullet.dstR->x > 2220 ||
      bullet.dstR->y > 1520) {
    // SDL_Log("Error: Bullet spawned with invalid coordinates!");
  } else {
    bullet.owner = EUID;
    bullet.damage = (damage * GameManagerSingleton::instance().getDifficulty());
    if (delayMS > 0) {
      ScheduledBulletList.emplace_back(std::pair<Bullet, int>(bullet, delayMS));
    } else {
      BulletList.emplace_back(bullet);
    }
    if (SoundCooldownBattlecruiser <= 0) {
      Sound::PlaySound(1);
      SoundCooldownBattlecruiser = 2;
    } else {
      SoundCooldownBattlecruiser--;
    }
  }
}
SDL_FRect BulletParticle;
void Bullet::updateBullets(GameManager &gm) {
  if (!ScheduledBulletList.empty()) {
    ...
    ScheduledBulletList.erase(
        std::remove_if(ScheduledBulletList.begin(), ScheduledBulletList.end(),
                       [](std::pair<Bullet, int> &scheduled) {
                         return scheduled.second <= -9999;
                       }),
      ...
      std::remove_if(
          ...
          [&gm](Bullet &bullet) {
            ...
            if (bullet.dstR == nullptr) {
              std::cout << "Bullet with wrong coordinates detected"
                        << std::endl;
              return true;
            }
            bullet.x +=
                bullet.velocity.x * GameManagerSingleton::instance().getDelta();
            bullet.y +=
                bullet.velocity.y * GameManagerSingleton::instance().getDelta();
            bullet.dstR->x = bullet.x;
            bullet.dstR->y = bullet.y;
            bullet.rotation += 1;

            if ((bullet.dstR->y < 0 || bullet.dstR->y > 1080) ||
                (bullet.dstR->x < 0 || bullet.dstR->x > 1920)) {
              return true;
            }
            return false;
          }),
