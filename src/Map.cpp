...
void Map::NewMap(int levelID) {
  ...
  while (std::getline(file, line)) {
    ...
    if (wPos != std::string::npos) {
      ...
      while (std::getline(waveStream, waveEntry, '.')) {
        ...
        EnemyWave wave;
        std::sscanf(typesStr.c_str(), "%d,%d", &wave.EnemyTypes[0],
                    &wave.EnemyTypes[1]);
        std::sscanf(countsStr.c_str(), "%d,%d", &wave.EnemyCount[0],
                    &wave.EnemyCount[1]);
  ...
  std::cout << "Config accepted for this level (" << levelID << ")"
            << std::endl;
}
void Map::MapGeneration() {
  ...
  struct WaveData {
    std::vector<std::pair<int, int>>
        enemyTypesAndCounts; // {{type1, count1}, {type2, count2}, ...}
    int delay;
  };
  ...
  for (int i = 0; i < 16; i++) {
    for (int j = 0; j < 4; j++) {
      ...
      if (i < 6) {
        if (j < 3) {
          enemies.emplace_back(Enemy::Randomizer(1, 2),
                               Enemy::Randomizer(1, 2));
          enemies.emplace_back(Enemy::Randomizer(1, 2),
                               Enemy::Randomizer(1, 2));
        } else {
          enemies.emplace_back(Enemy::Randomizer(3, 4), 1);
          enemies.emplace_back(Enemy::Randomizer(1, 2),
                               2 * Enemy::Randomizer(1, 3));
        }
      } else if (i < 10) {
        if (j < 3) {
          enemies.emplace_back(Enemy::Randomizer(1, 2),
  ...
  for (const auto &[levelID, mapIndex] : levelMapIndices) {
    ...
    for (size_t i = 0; i < waves.size(); ++i) {
      ...
      if (wave.enemyTypesAndCounts.size() >= 2) {
        configFile << wave.enemyTypesAndCounts[0].first << ","
                   << wave.enemyTypesAndCounts[1].first << ";";
        configFile << wave.enemyTypesAndCounts[0].second << ","
                   << wave.enemyTypesAndCounts[1].second << ";";
      } else {
...
void Map::MapUpdate(GameManager &gm) {
  src->w = static_cast<float>(gm.getWindowW());
...
float Map::lerp(float start, float end, float t) {
  t = 1.0f - std::pow(0.5f, GameManagerSingleton::instance().getDelta() * t);
  return start + t * (end - start);
}
void Map::MapRender(GameManager &gm) {
  if (MapTexture == nullptr) {
    MapTexture = TextureManager::LoadTexture(MapTextureNames[mapIndex]);
  }
  TextureManager::DrawMap(MapTexture, gm.getRenderer(), src, dst);
}
